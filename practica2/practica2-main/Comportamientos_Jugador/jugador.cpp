...
Action ComportamientoJugador::think(Sensores sensores)
{
	...
	current_node.nodoBasico.st = c_state;
	current_node.coste_total_secuencia = 0;
	
	current_node.tieneBikini = false;
	current_node.tieneZapatillas = false;

	if(mapaResultado[current_node.nodoBasico.st.jugador.f][current_node.nodoBasico.st.jugador.c] == 'K'){
		current_node.tieneBikini = true;
		current_node.tieneZapatillas = false;
	}
	else if (mapaResultado[current_node.nodoBasico.st.jugador.f][current_node.nodoBasico.st.jugador.c] == 'D')
	{
		current_node.tieneBikini = false;
    ...
    while (!frontera.empty() && !encontrado)
	{
		...
		if (current_node.nodoBasico.st.jugador.f == goal.f && current_node.nodoBasico.st.jugador.c == goal.c){
			encontrado = true;
			continue;
		}
		...
		for (auto accion : todas_acciones){
			...
			child_node.coste_total_secuencia += calculaCosteDelPaso(accion,mapaResultado[current_node.nodoBasico.st.jugador.f][current_node.nodoBasico.st.jugador.c],child_node);
			child_node.nodoBasico.st = apply(accion,current_node.nodoBasico.st,mapaResultado);

			if (mapaResultado[child_node.nodoBasico.st.jugador.f][child_node.nodoBasico.st.jugador.c] == 'K' && !child_node.tieneBikini){
				child_node.tieneBikini = true;
				child_node.tieneZapatillas = false;
			}
			else if (mapaResultado[child_node.nodoBasico.st.jugador.f][child_node.nodoBasico.st.jugador.c] == 'D' && !child_node.tieneZapatillas)
			{
	...
	plan = current_node.nodoBasico.secuencia;
	PintaPlan(plan);
...
list <Action> ComportamientoJugador::DijkstraLvl2(const state &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
    ...
    while (!frontera.empty() && !encontrado)
	{
		...
		for (auto accion : todas_acciones){
			...
			if (mapa[child_node.nodoBasico.st.jugador.f][child_node.nodoBasico.st.jugador.c] == 'K' && !child_node.tieneBikini){
				child_node.tieneBikini = true;
				child_node.tieneZapatillas = false;
			}
			else if (mapa[child_node.nodoBasico.st.jugador.f][child_node.nodoBasico.st.jugador.c] == 'D' && !child_node.tieneZapatillas)
			{
				child_node.tieneBikini = false;
				child_node.tieneZapatillas = true;
			}
...
list <Action> ComportamientoJugador::AnchuraLvl3(const state &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
	...
	node3 current_node;
    current_node.nodoBasico.st = inicio;
	current_node.coste_total = 0;
	current_node.coste_secuencia = 0;
	current_node.coste_heuristico = 0;
    ...
    while (!frontera.empty() && !haySolucion){
        ...
        for (auto accion : todas_acciones){
			...
			if (!clbEnVision && (accion == act_CLB_STOP || accion == act_CLB_WALK || accion == act_CLB_TURN_SR)){
				Action accion_new = current_node.nodoBasico.st.ultimaOrdenColaborador;
				child_node.coste_secuencia += calculaCosteDelPaso(accion_new,mapa[child_node.nodoBasico.st.colaborador.f][child_node.nodoBasico.st.colaborador.c],child_node);

				child_node.nodoBasico.st = apply2(accion_new,current_node.nodoBasico.st,mapa);
				child_node.nodoBasico.secuencia.push_back(accion_new);
				...
				child_node.coste_heuristico = costeHeuristico(child_node.nodoBasico.st,final);
			}
			else
			{
				if (accion == act_CLB_STOP || accion == act_CLB_WALK || accion == act_CLB_TURN_SR){
					child_node.coste_secuencia += calculaCosteDelPaso(accion,mapa[child_node.nodoBasico.st.colaborador.f][child_node.nodoBasico.st.colaborador.c],child_node);
				}
				else
				{
					child_node.coste_secuencia += calculaCosteDelPaso(accion,mapa[child_node.nodoBasico.st.jugador.f][child_node.nodoBasico.st.jugador.c],child_node);
				}
			...
			if (explorados.find(child_node) == explorados.end()){
				child_node.coste_total = child_node.coste_heuristico + child_node.coste_secuencia;
...
int ComportamientoJugador::costeHeuristico(const state &st, ubicacion posi){
	int df = abs(st.colaborador.f - posi.f);
    int dc = abs(st.colaborador.c - posi.c);
    return abs(dc+df);
}
