    ...
    while (!frontera.empty() && !haySolucion){
        ...
        for (auto accion : todas_acciones){
			...
			if (explorados.find(child_node) == explorados.end()){
				...
				child_node.coste_total_secuencia = current_node.coste_total_secuencia + costeHeuristico(child_node.nodoBasico.st,final) + calculaCosteDelPaso(accion,mapa[current_node.nodoBasico.st.jugador.f][current_node.nodoBasico.st.jugador.c],current_node);
