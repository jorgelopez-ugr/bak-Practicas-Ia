...
list <Action> ComportamientoJugador::AnchuraLvl1(const state &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
    ...
    while (!(current_node.st.colaborador.f == final.f && current_node.st.colaborador.c == final.c) && !cerrados.empty() && !haySolucion){
		...
		if (!(abiertos.find(current_node) == abiertos.end())){
			//si entra es porque esta repetido.
			continue;
		}
		
		abiertos.insert(current_node);

		// clbEnVision = colaboradorEnRangoVision(current_node.st.jugador,current_node.st.colaborador);
...
// bool ComportamientoJugador::seChocan(const node &n1 , const node &n2){
// 	if (n1.)
// }

//probando el pseudocodigo

// void ComportamientoJugador::programarAccionJugador(Action accion, const node &actual,const vector<vector<unsigned char> > &mapa){
// 	node nuevo = actual;
// 	nuevo.st = apply(accion,nuevo.st,mapa);

// 	if (!(nuevo.st == actual.st)){
// 		if (nuevo.st.ultimaOrdenColaborador != act_CLB_STOP){
// 			// nuevo.st = apply(accion,nuevo.st,mapa);

// 			if (!(nuevo.st == actual.st)){
// 				cerrados.push_back(nuevo);
// 				nuevo.secuencia.push_back(accion);
// 			}
// 		}
// 	}
// }
