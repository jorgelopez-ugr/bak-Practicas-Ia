...
list <Action> ComportamientoJugador::AnchuraLvl1(const state &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
    ...
    while (!(current_node.st.colaborador.f == final.f && current_node.st.colaborador.c == final.c)){
        ...
        for (auto accion : todas_acciones){
			...
			if (accion == current_node.st.ultimaOrdenColaborador){
				child_node.secuencia.push_back(actIDLE);
			}
			else
			{
