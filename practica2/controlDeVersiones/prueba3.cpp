...
list <Action> ComportamientoJugador::AnchuraLvl1(const state &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
	...
	{
		cerrados.pop_front();
		abiertos.insert(nodo_actual);

		if (nodo_actual.st.colaborador == final){
			haySolucion = true;
		}
		else
		{
			...
			if (enVision){
				node nodo_clbWalk = nodo_actual;
				nodo_clbWalk.st = apply(act_CLB_WALK,nodo_clbWalk.st,mapa);
				nodo_clbWalk.secuencia.push_back(act_CLB_WALK);

				if (nodo_clbWalk.st.colaborador == final){
					haySolucion = true;
					nodo_actual = nodo_clbWalk;
				}
				else if (abiertos.find(nodo_clbWalk) == abiertos.end())
				{
					cerrados.push_back(nodo_clbWalk);
				}

				if (!haySolucion){
					node nodo_clbSR = nodo_actual;
					nodo_clbSR.st = apply(act_CLB_TURN_SR,nodo_clbWalk.st,mapa);
					nodo_clbSR.secuencia.push_back(act_CLB_TURN_SR);

					if (abiertos.find(nodo_clbSR) == abiertos.end())
						cerrados.push_back(nodo_clbWalk);
			...
			{
				//Generar hijo actWALK
				node child_walk = nodo_actual; //se genera y luego se comprueba si tiene lo que hay que tener para pushearlo al frontier o no
				child_walk.st = apply(actWALK, nodo_actual.st, mapa); //comprueba si puede avanzar
				child_walk.secuencia.push_back(actWALK);
				if(colaboradorEnRangoVision(child_walk.st.jugador,nodo_actual.st.colaborador) && (abiertos.find(child_walk) == abiertos.end())){ // ¿hemos llegado?
					cerrados.push_back(child_walk);
					enVision = true;
				}
				//esto es asi porque se espera volver a el a comprobar.

				if(!enVision){ //en cada una de estas etas generando un nodo hijo que luego se va a explorar siguiendo el mismo paradigma
					//Generar hijo actRUN
					node child_run = nodo_actual;
					child_run.st = apply(actRUN, nodo_actual.st, mapa);
					child_run.secuencia.push_back(actRUN);
					if(colaboradorEnRangoVision(child_run.st.jugador,nodo_actual.st.colaborador)){
						nodo_actual = child_run;
						enVision = true;
					}
					else if(abiertos.find(child_run) == abiertos.end()){
						cerrados.push_back(child_run);
					}
				}
				...
				if (!enVision){ //en cada una de estas etas generando un nodo hijo que luego se va a explorar siguiendo el mismo paradigma
					//generar hijo de actTurn_L
					node child_turnl = nodo_actual;
					child_turnl.st = apply(actTURN_L,nodo_actual.st,mapa);
					child_turnl.secuencia.push_back(actTURN_L);

					if(colaboradorEnRangoVision(child_turnl.st.jugador,nodo_actual.st.colaborador)){
						nodo_actual = child_turnl;
						enVision = true;
					}
					else if(abiertos.find(child_turnl) == abiertos.end()){
						cerrados.push_back(child_turnl);
					}
					//generar hijo de actTurn_SR
					node child_turnsr = nodo_actual;
					child_turnsr.st = apply(actTURN_SR,nodo_actual.st,mapa);
					child_turnsr.secuencia.push_back(actTURN_SR);

					if(colaboradorEnRangoVision(child_turnsr.st.jugador,nodo_actual.st.colaborador)){
						nodo_actual = child_turnsr;
						enVision = true;
					}
					else if(abiertos.find(child_turnsr) == abiertos.end()){
						cerrados.push_back(child_turnsr);
					}
				}	
			}

			if (!haySolucion && !cerrados.empty()){ 
				nodo_actual = cerrados.front();

				while(!cerrados.empty() && abiertos.find(nodo_actual) != abiertos.end()){
					cerrados.pop_front();
