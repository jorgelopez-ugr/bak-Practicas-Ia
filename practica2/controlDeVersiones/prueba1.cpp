...
list <Action> ComportamientoJugador::AnchuraSoloJugador_V2(const stateN0 &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
...
list <Action> ComportamientoJugador::AnchuraSoloJugador_V3(const stateN0 &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
	...
	nodeN0 current_node;
	list<nodeN0> frontier; //cerrados
	set<nodeN0> explored; //abiertos
	...
	*/
	while(!frontier.empty() and !SolutionFound){ //mientras haya nodos que explorar y no se haya llegado a la solucion
		...
		//Generar hijo actWALK
		nodeN0 child_walk = current_node; //se genera y luego se comprueba si tiene lo que hay que tener para pushearlo al frontier o no
...
list <Action> ComportamientoJugador::AnchuraLvl1(const stateN0 &inicio, const ubicacion &final, const vector<vector<unsigned char> > &mapa){
    ...
    while (!esSolucion(current_node.st, final)){
        current_node = frontier.front();
        frontier.pop_front();
        ...
        for (auto accion : todas_acciones){
            ...
            if ((accion == act_CLB_WALK || accion == act_CLB_TURN_SR) && colaboradorEnRangoVision(current_node.st.jugador,current_node.st.colaborador)){
				child_node.st = apply(accion, current_node.st, mapa);
			}
			else if (accion != act_CLB_WALK && accion != act_CLB_TURN_SR)
			{
				if (colaboradorEnRangoVision(current_node.st.jugador,current_node.st.colaborador))
				{
					child_node.st = apply(actIDLE, current_node.st, mapa);
				}
				else
				{
					child_node.st = apply(accion, current_node.st, mapa);
...
bool ComportamientoJugador::veAlColaborador(Sensores sensores){
	for (int i = 0 ; i < sensores.agentes.size() ; i++){
		if (sensores.agentes[i] == 'c'){
			return true;
		}
	}
...
bool ComportamientoJugador::esSolucion(const stateN0 &actual, const ubicacion &final){

	if ((actual.jugador.f == final.f && actual.jugador.c == final.c)
	    ||(actual.colaborador.f == final.f && actual.colaborador.c == final.c)){
			return true;
	}
...
bool ComportamientoJugador::clbEnSensoresHipoteticos(const stateN0 &estado){
