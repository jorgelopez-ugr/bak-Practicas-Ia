...
class ComportamientoJugador : public Comportamiento{
  ...
  public:
    ComportamientoJugador(unsigned int size) : Comportamiento(size){
      ...
      lastPosi.first.first = 0;
      lastPosi.first.second = 0;
      lastPosi.second = false;
    ...
    vector <pair <pair<int,int>,bool>> buscaPuertas();
  ...
  pair < pair<int,int> , bool > lastPosi;

  //vector que guarda las posiciones de las puertas
  // y en el booleano guarda si debe ir a buscarlas o no
  vector <pair < pair<int,int> , bool>> registroPuertas;
  int indice_puerta;

  //matriz que guarda lo que ven los sensores para que en el momento
  //que bien_situado se ponga a true se complete automaticamente con lo
  //que ya conocia pero no sabia situar. Preciso saber que al momento
  //de reaparecer el agente siempre esta mirando al norte.
  vector< vector< unsigned char> > matrizAuxilar;
};
