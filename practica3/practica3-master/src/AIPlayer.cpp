...
void AIPlayer::think(color & c_piece,  int & id_piece, int & dice) const{ 
    ...
    switch(id){
        ...
        break;
        case 1:
            valor = Poda_AlfaBeta(*actual, jugador, 0, PROFUNDIDAD_ALFABETA, c_piece, id_piece, dice, alpha, beta, MiHeuristicaFinal);
        break; 
        case 2: 
...
double AIPlayer::Poda_AlfaBeta(const Parchis &actual, int jugador, int profundidad, int profundidad_max, color &c_piece, int &id_piece, int &dice, double alpha, double beta, double (*heuristic)(const Parchis &, int)) const {
    ...
    color last_color = c_piece;
    int last_piece = id_piece;
    int last_dice = dice;
    ...
    if (jugador == actual.getCurrentPlayerId())
    {
        ...
        {
            Parchis next_son = *it;
            aux = Poda_AlfaBeta(next_son, jugador, profundidad + 1, profundidad_max, c_piece, id_piece, dice, alpha, beta, *heuristic);
            if (alpha < aux)
            {
                alpha = aux;
                last_color = it.getMovedColor();
                last_piece = it.getMovedPieceId();
                last_dice = it.getMovedDiceValue();
            }
    ...
    else
    {
        ...
        {
            Parchis siguiente_hijo = *it;
            aux = Poda_AlfaBeta(siguiente_hijo, jugador, profundidad + 1, profundidad_max, c_piece, id_piece, dice, alpha, beta, *heuristic);
            if (beta > aux)
            {
                beta = aux;
                last_color = it.getMovedColor();
...
double AIPlayer::MiHeuristicaFinal(const Parchis &estado, int jugador){
    int ganador = estado.getWinner();
    int oponente = (jugador+1) % 2;
    ...
    else
    {
        ...
        for (int i = 0; i < my_colors.size(); i++)
        {
            ...
            for (int j = 0; j < num_pieces; j++)
            {
                puntuacionJugador += ( 70 - estado.distanceToGoal(c,j) * 3 );
                puntuacionJugador -= estado.piecesAtHome(c) * 20;
                ...
                {
                    puntuacionJugador += 40;
                }

                if (estado.isSafePiece(c,j)){
                    puntuacionJugador += 25;
                }

                int poder = estado.getPower(jugador);
                puntuacionJugador = puntuacionJugador + coeficientePoderes(poder);                
            }
        }
        ...
        for (int i = 0; i < op_colors.size(); i++)
        {
            ...
            for (int j = 0; j < num_pieces; j++)
            {
                puntuacionOponente += ( 70 - estado.distanceToGoal(c,j) * 3);
                puntuacionOponente -= estado.piecesAtHome(c) * 30;

                if (estado.getBoard().getPiece(c, j).get_box().type == goal)
                {
                    puntuacionOponente += 100;
                }

                if (estado.getBoard().getPiece(c, j).get_box().type == final_queue)
                {
                    puntuacionOponente += 40;
                }

                if (estado.isSafePiece(c,j)){
                    puntuacionOponente += 25;
                }

                int poder = estado.getPower(oponente);
                puntuacionOponente = puntuacionOponente + coeficientePoderes(poder);
            }
        }
...
int AIPlayer::coeficientePoderes(int poder){
    int solucion = 0;

    if (60 <= poder && poder < 65){
        solucion = -45;
    }
    else if (80 <= poder && poder < 85)
    {
        solucion = -70;
    }
    else if (90 <= poder && poder < 95)
    {
        solucion = -100;
    }
    else if (poder == 100)
